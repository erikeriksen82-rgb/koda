    ...
    <div className={`relative isolate min-h-screen overflow-hidden transition-colors duration-700 ${palette.surface}`}>
      ...
      >
        ...
        <header className="relative z-10 flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <h1 className="text-[2rem] font-semibold leading-[1.05] tracking-tight sm:text-[2.6rem] md:text-[3.25rem]">
                Vi bygger systemer og automatiser<br />arbeidsflyt med{" "}
        ...
        <div className="relative z-10 grid grid-cols-1 gap-4 md:grid-cols-3">
          {["AI Kartlegging", "System Kobling", "Automatisering"].map((label, index) => (
            <div key={label} onMouseMove={setSpotlight} onMouseLeave={clearSpotlight}
              className={`group relative overflow-hidden rounded-2xl border px-4 py-1.5 text-sm uppercase tracking-[0.3em] transition duration-500 ${palette.border} ${palette.highlight}`}>
              <span className={`block text-[10px] ${palette.subtle}`}>Funksjon</span>
              <span className="mt-2 block text-sm font-semibold tracking-[0.2em]">{label}</span>
              <span className="pointer-events-none absolute inset-0 opacity-0 transition duration-500 group-hover:opacity-100"
                style={{ background: "radial-gradient(220px circle at var(--spot-x, 50%) var(--spot-y, 50%), rgba(255,255,255,0.18), transparent 72%)" }} />
              <div className="pointer-events-none absolute -right-12 -top-12 h-32 w-32 rounded-full border border-current/15 opacity-70"
